...
#endif
	debug("%d, %d, %d", trigger_pin, echo_pin, max_cm_distance);
#endif
}
...
unsigned int NewPing::ping_median(uint8_t it) {
    unsigned int *uS = new unsigned int[it];
	unsigned int last;
	uint8_t j, i = 0;
	uS[0] = NO_ECHO;
	while (i < it) {
	...
	unsigned int median = uS[it >> 1];
	delete[] uS;

	return median; // Return the ping distance median.
}
...
#endif
#else
void NewPing::cbTimer() {
  static unsigned long milsec = 0;

  if (millis() - milsec >= 500000) {
	milsec = millis();
#endif
